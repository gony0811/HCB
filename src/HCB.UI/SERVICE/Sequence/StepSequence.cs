    ...
    public partial class SequenceService : BackgroundService
    {
        public async Task StepSequenceInitialize(CancellationToken ct)
        {
            try
            {
                ...
                await Task.Run(() => {
                    // 여기에 초기화 로직을 구현하세요.
                    this._sequenceServiceVM.StepWaferCenterMoveCompleted = StepState.Idle;
                    this._sequenceServiceVM.StepWaferMacroAlignCompleted = StepState.Idle;
                    this._sequenceServiceVM.StepWaferMicroAlignCompleted = StepState.Idle;
                    this._sequenceServiceVM.StepWaferCenterMoveElapsedTime = "00:00:00";
                    this._sequenceServiceVM.StepWaferMacroAlignElapsedTime = "00:00:00";
                    this._sequenceServiceVM.StepWaferMicroAlignElapsedTime = "00:00:00";
                });
        ...
        public async Task StepMoveWaferCenter(CancellationToken ct)
        {
            try
            {
                ...
                using (var linkedCts = CancellationTokenSource.CreateLinkedTokenSource(ct))
                {
                    ...
                    var measureTask = this._sequenceServiceVM.MeasureStepWaferCenterMove(linkedCts.Token);
